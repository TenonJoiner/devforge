...
文档评审——单轮全维度扫描 + 人工决策门禁。默认自动发现 openspec/changes 下的 change 目录；带 `autofix` 时自动修复并循环评审。

## 使用场景

- **手动触发**：`/df:spec-review` 自动发现 openspec/changes 下的合适 change；或 `/df:spec-review --change-dir <path>` 指定具体目录做临时体检
