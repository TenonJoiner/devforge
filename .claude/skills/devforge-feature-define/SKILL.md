...
- 特性级（本 skill）：承接已识别 Actor，在既有 Feature 框架内定义 Requirement + Scenario，产出 `openspec/changes/<name>/specs/*.md`
...
读取 `openspec/changes/<change-name>/specs/` 目录：
- **不存在或为空** → 进入「初次生成」模式
...
```
当前是特性级 specs 阶段，为 OpenSpec change「<change-name>」生成 specs/<capability>/spec.md。

**任务**：为 Capability「<capability>」定义 Requirement + Scenario。

**输入**：
- proposal.md：<路径>（读该 Capability 的描述）
- research.md：<路径>（读约束清单，作为 Requirement 边界）
- 产品级需求文档：docs/requirements/<相关 Feature>.md（获取 Actor 清单和验收标准）
- spec.md template：openspec/schemas/spec-driven-enhanced/templates/spec.md
...
**被评审对象**：<路径>
**被评审 template**：openspec/schemas/spec-driven-enhanced/templates/spec.md
