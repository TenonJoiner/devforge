...
**语言/领域特有加项**（根据 domain-config.yaml 调整）：
- **C/C++**：检查是否引入了新的内存分配模式而项目内已有统一内存池、检查锁/无锁结构的使用是否已有更成熟的内部封装可用
- **分布式系统**：检查序列化/反序列化是否可被延迟或批量、检查故障恢复路径是否引入了全量扫描而非增量
- **数据库**：检查查询优化器是否可缓存执行计划、检查索引是否可批量更新
- **编译器**：检查 AST 遍历是否可合并、检查优化 pass 是否可延迟执行
...
**语言/领域特有加项**（根据 domain-config.yaml 调整）：
- **C/C++**：检查 `memcpy` 是否可用零拷贝替代、检查大锁是否保护了无共享状态的路径
- **分布式系统**：检查序列化/反序列化是否可被延迟或批量、检查故障恢复路径是否引入了全量扫描而非增量
- **数据库**：检查查询优化器是否可缓存执行计划、检查索引是否可批量更新
- **编译器**：检查 AST 遍历是否可合并、检查优化 pass 是否可延迟执行
