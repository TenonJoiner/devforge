...
- **原始 agent**（researcher / product）：将完整产出写入 `-draft-` 标记的中间文件，向主会话返回轻量摘要（≤5 行：关键数字、覆盖率、异常点）
...
| 4 路 agent 产出 Actor 候选需合并 | ✅ 派新 product agent 读取 `actors-draft-*.md`，合并后 Write `product-spec.md#Actors` | ❌ 主会话读内容做合并决策并写入 |
| product-spec.md 篇幅 600-900 行 | ✅ 合并 product agent 用 Write 写入，主会话只确认完成状态 | ❌ 主会话自己 Write 大文档 |
| 评审完成后写评审记录 | ✅ reviewer 追加到 `-review.md`，合并 agent 将评审状态写入最终文件 | ❌ 主会话读评审内容后写入 |
| reviewer 提出 Actor 描述需修正 | ✅ 派 product agent 读取文件 + review 文件后 Edit 修正 | ❌ 主会话直接改写文件内容 |

**关键判定**：
- 原始产出（调研/识别/挖掘）→ 由原始 agent 写入 `-draft-` 文件
- 合并去重 → 由**同类型新 agent 实例**执行（保持领域一致性）
- 评审意见 → 由 reviewer 追加到 `-review.md` 文件
- 最终文件 → 由合并 agent 写入

**禁止**：
- 主会话直接读取 agent 的完整产出让上下文膨胀（只允许读取 ≤5 行轻量摘要）
- 主会话实质性修改 agent 产出的核心内容
- 无 agent 执行记录就声称完成
- 用单 agent 内部发散替代多 agent 独立产出
- 合并 agent 与原始 agent 类型不一致（如用 researcher 合并 product 的 draft）
...
- **Reviewer agent**：将逐条评审意见追加到 `-review.md` 文件（如 `reference/<product>-review.md`、`product-spec-review.md`）
