...
> **强制约束**：合并决策与文件写入均由主会话亲自执行，**禁止派 agent 重新合并或代写文件**（参见 SKILL.md「主会话职责边界」）。
...
**Step 4 主会话用 Write 工具写入** `docs/requirements/product-spec.md`（Actors 章节），格式遵循 `.claude/templates/req-product-spec.md`：
- 文档篇幅大时分多次 Write+Edit 拼接，上下文紧张先 /compact 释放空间
...
- 产出 Feature 间依赖关系矩阵
- **主会话用 Edit 工具**写入 `product-spec.md` Feature 章节草稿（复杂度字段先留空）
  > **强制约束**：合并决策与文件写入均由主会话亲自执行，**禁止派 agent 重新合并或代写章节**（参见 SKILL.md「主会话职责边界」）
...
- **主会话用 Edit 工具**将复杂度评估回填到 `product-spec.md` 对应 Feature 字段
  > **强制约束**：回填操作由主会话亲自执行，禁止派 agent 代写
