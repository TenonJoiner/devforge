...
3. **分析 Lint 报告**（仅部分 lint 命令存在告警时执行）
   ...
   **c. 交叉判定**：结合源码上下文和规则意图，逐条产出判定结果（非最终报告）：
   ...
   | 分类 | 处理 |
   |------|------|
   | 需修复 + 级别确认 | 保留，输出问题说明和修复建议，级别字段显示判定级别 |
   | 需修复 + 级别调整 | 保留，级别字段显示"原级别 → 建议级别"，调整理由写入问题说明 |
...
所有模式下，除会话内输出外，必须将报告写入 `/tmp/lint-report-<YYYYMMDD-HHMMSS>.md`。每次执行生成新文件，不覆盖历史报告。
...
Lint 分析报告
  ...
  历史遗留 N 条：
    1. [文件:行号] <告警内容>
       问题: <问题说明>
       修复: <修复建议>
       规则: <规则名> (<一句话说明规则含义>)    级别: <判定级别>    标记: 历史遗留，不阻塞本次
       ```<lang>
       ...
       ```
