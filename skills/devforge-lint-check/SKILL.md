...
3. **分析 Lint 报告**（仅部分 lint 命令存在告警时执行）
   ...
   原始 lint 报告存在噪声——告警不一定是真问题，级别也不一定准确。将告警按文件分组后派遣 devforge:lint-analyst 并行分析（告警 ≤ 3 条且 ≤ 2 个文件时 agent 自行分析），逐条经**源码 + lint 规则**交叉验证后，**废弃原始报告，按输出格式重写新报告**。禁止在原报告上逐条追加判定标注——必须按分类（需修复/历史遗留/已排除/规则建议）重新组织内容。

   每条告警，devforge:lint-analyst 执行以下三步：
