...
| **Command** | 用户主动触发的入口点（`/opsx:*` 触发 Schema，`/df:*` 触发 Skill） | 全层级 |
...
| 产品级探索 skill | `/df:explore` 执行一轮架构探索 | 启发式思考引导有效，输出 ADR 和架构文档 |
| 产品级评审 skill | `/df:product-review` 评审产品级文档 | 跨文档一致性检查和红旗检测正常工作 |
| 产品级一致性检查 | `/df:verify` 执行产品级 vs 特性级一致性检查 | 输出不一致清单和方向建议 |
| 代码级 TDD | `/df:tdd` 执行一个完整 RED-GREEN-REFACTOR 循环 | 测试先红后绿 |
