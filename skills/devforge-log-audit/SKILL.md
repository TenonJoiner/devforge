...
parameters:
  - name: autofix
    description: 审计后自动修复日志问题（默认只审计不修复）
    required: false
    default: false
  - name: full
...
审计知识基线（级别语义 + 反模式 + 判定阈值）内建于 `devforge:log-auditor` agent；**项目特定级别定义由第 1 阶段探测后注入 `devforge:log-auditor`**。默认只审计不修复；带 `--autofix` 时自动派遣 developer 修复并回归验证，最多 5 轮。

### 职责边界

- ✅ 探测范围内各语言的日志级别定义（配置/文档/源码反推）与日志框架
- ✅ 派遣 `devforge:log-auditor` 执行审计（无 `--log-dir` 时仅级别审计，有时两维度均执行），汇总分级报告
- ✅ `--autofix` 模式下派遣 developer 按报告修复日志问题，修复后回归审计验证
...
- [ ] 第 3 阶段独立审核已完成：草稿问题清单逐条验证，误报已移除，级别已校准，设计确认已移入级别使用分析，内部一致性已检查
- [ ] 报告按 CRITICAL/HIGH/MEDIUM/LOW 分级，结论标注 PASS / NEEDS-FIX
- [ ] `--autofix` 模式下：修复循环不超过 5 轮，CRITICAL/HIGH 已清零或达上限后标注残留
- [ ] **报告格式校验**：主会话对最终报告做轻量校验——必填章节全部存在 + 四级问题格式统一 + 每条含编号/标题/维度/说明/修复建议 + CRITICAL/HIGH 条目含预期收益 + 数字摘要与问题清单各级别条目数一致
...
| 🚩 修复死循环 | `--autofix` 模式下 5 轮修复后仍有未解决的 CRITICAL/HIGH | 停止，输出完整问题清单 |
| 🚩 修复引入新问题 | developer 修复后出现新的 CRITICAL/HIGH | 继续修复循环（消耗轮次），若同时触发 5 轮上限则停止 |
...
- **接受修复**：对 CRITICAL/HIGH 项逐条确认后手动修复
- **接受风险**：显式记录某条不修复的原因，由 `devforge:log-auditor` 误报则反馈改进基线
- **批量整改**：全仓场景产出整改清单后，按模块分批修复

### `--autofix` 模式

审计报告通过格式校验后，若报告结论为 NEEDS-FIX（存在 CRITICAL 或 HIGH），进入修复循环：

1. **派遣 developer 修复**：读取最终报告，按 CRITICAL → HIGH → MEDIUM 优先级修复日志语句（改级别、换宏、加限速参数），修复后写回源码
2. **回归审计**：对修复后的变更重新执行审计（仅审计本次修复引入的变更），验证问题是否已消除
3. **循环控制**：最多 5 轮；无 CRITICAL/HIGH 残留时收敛退出；达到上限仍未清零则停止，标注残留问题
