...
5. **所有内容结构严格遵循 `templates/req-feature.md` 模板**
6. 量化非功能需求
...
- **Scenario 产出格式为表格行**（场景一句话描述 + 量化验收标准 + 可测试性），禁止展开为多段式用例（前置条件/触发动作/预期行为/验证方法等字段留给特性级阶段）
...
- **被评审 template 路径**：`templates/req-feature.md`（视角来源 1：reviewer 从其 mandatory-sections + 自检清单提取评审锚点）
