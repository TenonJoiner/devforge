...
| A2 `developer` | — | 生产代码 `/opsx:apply`：TDD 循环 + 重构 | Q.1-Q.2 编译+单测；夜间 CI 修复 | `/ky:tdd` `/ky:refactor` |
| A3 `code-reviewer` | — | 生产代码 `/opsx:apply`：代码检视 (N.M.6) | Q.4 全量 diff 评审 | `/ky:review` `/ky:spec-review` |
| A4 `debugger` | — | — | 夜间 CI 复杂失败：PM 分派介入定位根因 | `/ky:debug` |
| A5 `frontend-developer` | — | 前端代码 `/opsx:apply`：TDD + 重构 + 评审（全周期） | — | `/ky:tdd`（前端代码上下文） |
...
> **测试验证级不设 `/ky:*` 命令**：测试用例开发在 `tests/` 目录下走 OpenSpec workflow（由 `integration-tester`/`perf-tester` 作为执行 Agent），测试执行通过脚本触发，不需要专门的 skill 和 command。
