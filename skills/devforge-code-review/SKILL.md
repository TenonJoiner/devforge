...
2. **逐条复核**：对草稿中每条 CRITICAL/HIGH/MEDIUM 发现，结合源码判定：
   - **KEEP（默认）**：原级别不变，保留在原级别区块。证据不足以证伪或调整级别时一律保留原级别。**存疑保留，宁可误报不可漏报**——漏掉真问题比多报一条噪声代价更高。
   - **降级**：实际危害低于原级别（如 CRITICAL→HIGH、HIGH→MEDIUM），归入降级后的级别区块。
   - **升级**：实际危害高于原级别（如 MEDIUM→HIGH、HIGH→CRITICAL），归入升级后的级别区块。
   - **排除**：仅当能用源码证据**正面证伪**——发现基于误读代码、前置条件在上游已满足、路径实际不可达，或属「假阳性排除规则」已覆盖的场景。
   - LOW 与变体发现不参与复核（本就不阻塞合并）。
3. **产出终稿**：审核者按**最终级别**重写报告——保留的发现归入各自最终级别区块，按最终级别重新编号（如 HIGH 降为 MEDIUM 后编号变为 M 前缀）；被排除的发现移入「已排除」附录，按排除原因归类简要总结，不逐条展开。终稿中**禁止出现**原始级别标记、审核变更元数据（如"原 HIGH""降级 1 条""排除 3 条"等标注）——报告呈现为一份干净的直接评审结果，不暴露审核过程。
...
> 误报审核环节从初评草稿中剔除的发现，按排除原因归类简要总结。不阻塞合并、不计入 PASS/NEEDS_FIX 判定。

- **[排除原因]**：N 条 — [共同排除理由 + 代表性源码依据]
- ...
```
