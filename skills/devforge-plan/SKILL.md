...
> **内容规范强牵引**：计划交付件的所有内容结构、章节要求、自检清单**以 `templates/*.md` 为唯一准绳**。本 skill 只负责定义**努力程度、写作风格、质量门槛和流程控制**，不再重复模板中已有的内容要求。
...
7. **产品级颗粒度**：只规划"做什么/何时做/什么顺序做"，特性级细节（接口签名、字段、配置参数等）不在 `/df:plan` 提前做
...
2. **被评审 template 路径**（如 `templates/plan-milestone.md` / `plan-iteration.md`，指示视角来源 1）
