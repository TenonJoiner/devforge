...
2. 读取 `templates/arch-adr.md`，明确模板章节结构
3. **决定 agent 配置**：
...
- **输入**：`adr-draft-*.md`
- **执行**：
  - 读取所有 draft，按 `templates/arch-adr.md` 模板结构整合
...
2. 读取 `templates/arch-system.md`，明确模板章节结构
3. **决定 agent 配置**：
   - **architect 数量**：极高复杂度下限 ≥5（阶段总 agent 数）
...
- **输入**：`design-draft-*.md`
- **执行**：
  - 读取所有 draft，按 `templates/arch-system.md` 模板结构整合
  - 确保各子系统职责明确无重叠或遗漏
  - 数据流正常路径 + 错误传播路径完整定义
  - 与 `adr.md` 各 ADR 决策一致
...
3. 读取 `templates/arch-subsystem.md`，明确模板章节结构
4. **决定 agent 配置**：
...
- **执行**：
  - 读取所有 draft，按 `templates/arch-subsystem.md` 模板结构整合
  - 核心数据流、状态转换、接口契约完整
  - 故障模式、降级策略、恢复流程已定义
