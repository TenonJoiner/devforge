...
**模板**：`templates/ref-architecture.md`
...
2. 读取 `templates/ref-architecture.md`，明确模板章节结构
3. **决定研究策略**：
...
1. 按 2.1 确定的配置，**每个标杆启动 ≥2 个 researcher agent**：
   ...
   - 模板约束：使用 `templates/ref-architecture.md`
...
- **执行**：读取所有 draft，按以下规则整合为单一文档
  - 按 `templates/ref-architecture.md` 模板组装
...
- **被评审 template 路径**：`templates/ref-architecture.md`（视角来源 1：reviewer 从其 mandatory-sections + 深度自检清单提取评审锚点）
