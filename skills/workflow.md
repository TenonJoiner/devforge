...
```
产品级（启发式 Skills + /ky: 命令，与 OpenSpec 脱离）──────────────
  /ky:explore ←→ /ky:define ←→ /ky:plan（反复迭代，无固定顺序）
      │               │              │
      ▼               ▼              ▼
...
代码级（teamskills /ky:* skills）──────────────────────────────
  worktree 隔离 → /ky:tdd(R→G→R) → /ky:refactor → /ky:lint
  → /ky:review → commit
          │                                          │
          │ 单元测试通过                                │ 触发测试验证级
          ▼                                          ▼
测试验证级（独立测试目录 workflow（待定义）+ 脚本执行）─────────────
  ...
  → 脚本执行（集成测试 + 性能测试）→ merge
       ...
       ┌──── 变更反馈回路 ──────────────────────────────┘
       ▼
  /ky:verify: 全量双向一致性检查（产品级文档 ↔ 特性级 spec/design/code）
```

> **四层边界说明**：代码级聚焦"写对代码"（单元测试验证函数/模块内部逻辑），测试验证级聚焦"集成正确"（验证跨子系统交互、端到端行为、性能指标）。两层通过 commit 事件衔接。测试用例开发在独立测试仓中走 OpenSpec workflow，测试执行通过脚本触发，不设专门的 `/ky:*` 命令。
...
**触发方式**：`/ky:explore`、`/ky:define`、`/ky:plan`、`/ky:product-review`、`/ky:test-design`、`/ky:verify`

典型迭代流程（非强制顺序，可反复执行）：

1. `/ky:explore` → 探索架构方案、竞品分析、子系统分解
...
5. `/ky:product-review` → 多维评审所有产品级文档（含 test-strategy.md 和各级测试方案）
   - 跨文档一致性 + 完备性 + 红旗检测
   - 两轮评审：AI 评审 + 人员交叉评审
6. `/ky:verify` → 全量双向一致性检查
...
1. 生成 proposal.md（遵循产品级约束，追溯到 iteration-plan 条目）
   - `/ky:spec-review` → 检查 proposal 是否符合产品级需求
2. `/opsx:continue` → 生成 specs/\*\*/\*.md（Delta 格式特性规格）
   - `/ky:spec-review` → 检查 spec 是否完整可验收
3. `/opsx:continue` → 生成 design.md（技术设计）
   - `/ky:spec-review` → 检查 design 是否可实现
...
5. `/opsx:continue` → 生成 tasks.md（TDD 粒度任务清单，前置条件：leader 签字 APPROVED）
   - `/ky:spec-review` → 检查 tasks 分解是否合理
6. `/opsx:apply` → 按 tasks.md 逐条实现（调度代码级 skills）
7. `/opsx:verify` → 三维验证（完整性/正确性/一致性）
8. `/opsx:archive` → 合并 Delta + 归档

> `/ky:spec-review` 与 review artifact 的区别：

| 维度 | `/ky:spec-review`（S3，轻量质检） | review artifact（正式评审门禁） |
|------|----------------------------------|-------------------------------|
| **触发方式** | 手动调用 `/ky:spec-review`，每个 `/opsx:continue` 前可选执行 | `/opsx:continue` 自动生成 review.md |
...
**触发方式**：`/opsx:apply` 阶段自动调度，或手动 `/ky:tdd`
...
/opsx:apply
  ...
  │ │  （/ky:switch-worktree 切换工作区）
  │ │
  │ │ 遍历每条实现 task：
  │ ├─→ /ky:tdd      RED-GREEN-REFACTOR 循环（N.M.1 ~ N.M.4）
  │ ├─→ /ky:refactor  代码简化重构（N.M.5）
  │ ├─→ /ky:review    代码检视（N.M.6）
...
1. TDD 铁律循环（/ky:tdd）：
   ...
   - REFACTOR: `/ky:refactor` 代码简化重构 → VERIFY-GREEN: 再次确认
2. `/ky:lint` 编译检查（`gcc -Wall -Wextra -Werror`）+ clang-tidy 静态分析
3. `/ky:review` 三级代码评审（通用检视 → C 语言专项 → 安全审计）
...
> `/opsx:apply` 是特性级的**编排入口**，代码级的 `/ky:*` skills 是**被调度的执行单元**，两者是调用者与被调用者的关系。代码级的每个 `/ky:*` 命令也支持开发者手动直接调用，覆盖不经过 OpenSpec 的场景（如修 bug、写工具函数）。并行开发（parallel skill）和 worktree 创建由 `/opsx:apply` 内部调度，不设独立命令。
...
测试通过并合并后，通过 `/ky:verify`（PS6）执行全量双向一致性检查，识别产品级文档与特性级 spec/design/code 之间的不一致，确保上下层文档同步更新。
