...
> 产品级 6 个启发式 skill（按思考模式拆分），含 `product/verify` 解决变更回溯问题（P5）。代码级 8 个 skill，含 `code/spec-review` 覆盖特性级交付件评审（P2+P6）和 `code/systematic-debug` 覆盖系统化调试（P3）。测试验证级不设独立 skill——测试策略设计由 `product/test-design` 调度测试 Agent 执行，测试用例开发在独立测试仓走 OpenSpec workflow（由 `integration-tester`/`perf-tester` 作为执行 Agent），测试执行通过脚本或夜间 CI 触发。
...
| PS3 | `product/plan` | 规划性思考 | iteration-plan.md（含测试代码仓 proposal） | MVP 识别 + 依赖图分析 + 并行分组（Wave）+ 复杂度估算（S/M/L）+ 递归深化 + 测试代码仓 proposal 生成。**执行角色**：`project-manager`(A10) 编排 + `architect`(A1) 技术可行性判断 |
...
| A4 | `debugger` | 调试工程师：系统化调试四阶段（根因调查→多分量诊断→数据流追踪→验证修复）。**铁律：NO FIX WITHOUT ROOT CAUSE**。关注内存损坏定位（ASan/valgrind）、并发死锁分析（helgrind/锁序图）、网络协议异常追踪、状态机不一致诊断、gdb/strace/core dump 分析。**使用时机**：手动 `/ky:debug` 触发 + 夜间 CI 复杂失败时由 PM 分派介入（developer 无法直接修复的场景） | sonnet | P3 | superpowers/systematic-debugging |
| A5 | `frontend-developer` | 前端开发工程师：管理工具/CLI/UI 开发、代码评审。**关注集群拓扑可视化、存储容量/性能监控展示、运维操作安全确认**。在前端仓的 `/opsx:apply` 中担任执行 Agent（兼任开发+评审，因前端代码不适用 A3 的 C 语言专项评审管线） | sonnet | P3 | everything-claude-code/code-reviewer |
...
|-------|--------|--------|-------------------|------------|
| A1 `architect` | PS1 explore + PS2 define + PS4 review + PS6 verify 执行角色 | — | — | `/ky:explore` `/ky:define` `/ky:product-review` `/ky:verify` |
| A2 `developer` | — | 生产仓 `/opsx:apply`：TDD 循环 + 重构 | Q.1-Q.2 编译+单测；夜间 CI 修复 | `/ky:tdd` `/ky:refactor` |
| A3 `code-reviewer` | — | 生产仓 `/opsx:apply`：代码检视 (N.M.6) | Q.4 全量 diff 评审 | `/ky:review` `/ky:spec-review` |
| A4 `debugger` | — | — | 夜间 CI 复杂失败：PM 分派介入定位根因 | `/ky:debug` |
| A5 `frontend-developer` | — | 前端仓 `/opsx:apply`：TDD + 重构 + 评审（全周期） | — | `/ky:tdd`（前端仓上下文） |
| A6 `integration-tester` | PS5 集成/系统测试方案设计 | 测试仓 `/opsx:apply`：编写集成/系统测试用例 | Q.3 冒烟验证；夜间 CI 执行+分析 | — |
| A7 `perf-tester` | PS5 性能测试方案设计 | 测试仓 `/opsx:apply`：编写性能测试用例 | 夜间 CI 执行+分析 | — |
...
| 阶段 | Agent | 做什么 | 触发方式 |
|------|-------|-------|---------|
| **架构探索** | `architect`(A1) | 作为 PS1 `product/explore` 执行角色：竞品分析、多方案探索、质量属性评估、ADR 输出 | `/ky:explore` |
| **需求定义** | `architect`(A1) | 作为 PS2 `product/define` 执行角色：Feature-Scenario 分解、接口契约设计、非功能需求量化 | `/ky:define` |
| **迭代计划** | `project-manager`(A10) + `architect`(A1) | A10 编排计划结构（MVP 识别、Wave 分组、复杂度估算），A1 提供技术可行性判断和依赖分析 | `/ky:plan` |
| **文档评审** | `architect`(A1) 主导 | 作为 PS4 `product/review` 执行角色：跨文档一致性检查、完备性检查、红旗检测 | `/ky:product-review` |
| **一致性验证** | `architect`(A1) 主导 | 作为 PS6 `product/verify` 执行角色：产品级文档 vs 特性级 spec/design/code 的双向一致性检查 | `/ky:verify` |

**测试 Agent（A6 + A7）**：

| 阶段 | Agent | 做什么 | 触发方式 |
|------|-------|-------|---------|
| **产品级：测试策略设计** | `integration-tester`(A6) + `perf-tester`(A7) | 作为 PS5 执行角色：A6 设计集成/系统测试方案（组件交互、故障注入、一致性验证），A7 设计性能测试方案（基准指标、压力场景、回归检测阈值）。输出纳入 test-strategy.md | `/ky:test-design` 内部调度 |
| **特性级：测试用例开发** | `integration-tester`(A6) 或 `perf-tester`(A7) | 在独立测试仓的 `/opsx:apply` 中作为执行 Agent（替代生产代码仓中 `developer` 的角色），编写集成/性能测试代码。遵循 R4 `testing.md` 中的 Mock 纪律、环境搭建/拆除、故障注入验证标准 | 测试仓的 `/opsx:apply` |
...
| 场景 | 触发条件 | 做什么 |
|------|---------|-------|
| **手动调试** | 开发者遇到难以定位的问题，执行 `/ky:debug` | 系统化调试四阶段：根因调查→多分量诊断→数据流追踪→验证修复 |
...
> A5 在前端仓（管理工具/CLI/UI）的 `/opsx:apply` 中担任全周期执行 Agent——兼任开发（TDD）+ 评审。因前端代码不适用 A3 的 C 语言三级评审管线（内存安全/并发/指针检查），由 A5 自行完成前端相关的代码评审（可读性/组件结构/状态管理/UI 一致性）。
...
## 七、Commands 清单（13 个，/ky: 前缀）

> `/ky:parallel` 和 `/ky:worktree`（创建）仅作为 Skill 存在，由 `opsx:apply` 内部调度，不设独立命令。`/ky:switch-worktree` 供用户手动切换 worktree 或切回主干。

teamskills 的自建命令使用 `/ky:` 前缀，与 OpenSpec 的 `/opsx:` 前缀区分。

### 7.0 产品级 Commands（6 个，解决 P1+P4+P5）

| # | Command | 用途 | 解决 | 映射 Skill |
|---|---------|------|------|-----------|
| C1 | `/ky:explore` | 探索架构方案、竞品分析、子系统分解 | P1 | product/explore |
| C2 | `/ky:define` | 定义需求、接口契约、验收标准 | P1 | product/define |
| C3 | `/ky:plan` | 制定迭代计划和 proposal 清单 | P1 | product/plan |
| C4 | `/ky:product-review` | 多维评审产品级文档 | P1,P6 | product/review |
| C5 | `/ky:test-design` | 制定测试策略和具体测试方案 | P4 | product/test-design |
| C6 | `/ky:verify` | 全量双向一致性检查：产品级文档 vs 特性级 spec/design/code | P5 | product/verify |

### 7.1 代码级 Commands（7 个）

| # | Command | 用途 | 解决 | 映射 Skill |
|---|---------|------|------|-----------|
| C7 | `/ky:tdd` | 启动 TDD 开发流程（RED-GREEN-REFACTOR） | P3 | code/tdd-workflow |
| C8 | `/ky:review` | 启动代码评审（三级管线） | P3,P6 | code/code-review |
| C9 | `/ky:spec-review` | 特性级交付件评审（proposal/spec/design/tasks） | P2,P6 | code/spec-review |
| C10 | `/ky:refactor` | 启动代码重构（保持测试绿色） | P3 | code/code-refactor |
| C11 | `/ky:lint` | 运行编译检查 + 静态分析 | P3 | code/lint-check |
| C12 | `/ky:switch-worktree` | 切换到指定 worktree 或切回主干（更新 `.claude/active-worktree` 状态文件，配合 worktree-guard hook 实现写操作守护） | P3 | code/git-worktree |
| C13 | `/ky:debug` | 启动系统化调试流程（根因调查→多分量诊断→数据流追踪→验证修复） | P3 | code/systematic-debug |

### 与 OpenSpec 命令的分工

| 层级 | 命令前缀 | 示例 |
|------|---------|------|
| 产品级 | `/ky:` | `/ky:explore`、`/ky:define`、`/ky:plan`、`/ky:product-review`、`/ky:test-design`、`/ky:verify` |
| 特性级 | `/opsx:` | `/opsx:propose feature-xxx`、`/opsx:continue`、`/opsx:apply`、`/opsx:verify`、`/opsx:archive` |
| 代码级 | `/ky:` | `/ky:tdd`、`/ky:review`、`/ky:spec-review`、`/ky:refactor`、`/ky:lint`、`/ky:switch-worktree`、`/ky:debug` |

> **测试验证级不设 `/ky:*` 命令**：测试用例开发在独立测试仓中走 OpenSpec workflow（由 `integration-tester`/`perf-tester` 作为执行 Agent），测试执行通过脚本触发，不需要专门的 skill 和 command。

### `/opsx:apply` 与代码级 Skills 的关系

`/opsx:apply` 是特性级的**编排入口**，代码级的 `/ky:*` skills 是**被调度的执行单元**。两者是调用者与被调用者的关系，不是并列关系。
...
| 机制 | 说明 |
|------|------|
| **Skill 引用** | `/opsx:apply` 的 instruction 中通过 `/ky:tdd`、`/ky:refactor`、`/ky:review` 等命令名引用代码级 skill。Claude Code 自动加载对应 SKILL.md |
...
代码级的每个 `/ky:*` 命令也支持开发者手动直接调用，覆盖不经过 OpenSpec 的场景（如修 bug、写工具函数）。此时开发者自行决定调用哪些 skills，不受 `opsx:apply` 编排。

---

## 八、Hooks 清单（3 条）

> 只保留真正有价值且不扰民的 hook。变更回溯统一由 `/ky:verify`（PS6）主动触发。
...
| H3 | `worktree-guard` | PreToolUse(Edit/Write) | **worktree 写操作守护**：读取 `.claude/active-worktree` 状态文件，当存在活跃 worktree 时，仅放行活跃 worktree 目录内和项目外的写操作，拦截对主干代码及其他非活跃 worktree 的写操作（读操作不拦截，避免高频触发的性能开销）。`/ky:switch-worktree` 执行时自动更新状态文件，guard 随之切换守护范围。防止会话中断、compaction 导致记忆丢失后误操作 | P3 | 自创 |
...
|---|------|---------|------|------|---------|
| R1 | `workflow.md` | 无（全局） | 四层工作流规范（产品级→特性级→代码级→测试验证级）、OpenSpec 与 teamskills 的分工、审批节点、子系统分工、接口对齐、冲突解决、并行开发规约、**漂移信号检测**：`/opsx:archive` 归档时扫描「新增发现」「架构偏离」标签，累计超过阈值（3 个未处理）时提示执行 `/ky:verify` | P1,P2,P3,P4,P5 | OpenSpec schema + gstack + SuperClaude + superpowers |
