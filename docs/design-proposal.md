...
**核心决策**：安装 OpenSpec 作为基础，利用其特性级 workflow 引擎（命令系统、依赖图、Schema 自定义、模板、Delta spec、archive），不重建这些基础设施。特性级的工作流表达为 OpenSpec Schema（spec-driven-enhanced），代码级使用 Claude Code 原生扩展（skills/agents/commands/hooks/rules）自建，测试验证级在独立测试仓中走专用 workflow（待定义，不复用功能代码仓的 spec-driven-enhanced）。产品级与 OpenSpec 彻底脱离，使用 6 个启发式 skill（按思考模式拆分：explore/define/plan/review/test-design/verify）辅助产品级设计——因为产品级设计是持续数月的创意迭代过程，重要的是**输出质量**而非**流程控制**。不安装 superpowers，但借鉴其 TDD、Subagent、计划编写等理念。
...
| **[团队协作与变更同步](team-and-sync.md)** | 多仓模型 + 并行开发 + 安装机制 + 变更回溯机制 |
