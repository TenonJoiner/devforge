...
**触发命令**：
- `/ky:architect` — 架构探索（检测现有文档，支持迭代完善）
- `/ky:define` — 需求定义（Feature-Scenario 结构，接口契约）
- `/ky:plan` — 迭代计划（MVP → Wave 分组 → 关键路径）
...
4. design.md 自动关联产品级架构文档（`docs/architecture/design.md` 系统总纲，及 `docs/architecture/<subsystem>/*.md` 相关子系统）
...
- 累计超过 3 个时，提示执行 `/ky:architect` 或 `/ky:define` 更新产品级文档
...
| design.md | architecture/design.md（系统总纲）、architecture/<subsystem>/*.md（相关子系统） |
...
```
第 1-2 周：/ky:architect → 系统级架构框架
第 2-4 周：/ky:define → 核心 Feature 需求
第 3-5 周：/ky:plan → 初始迭代计划
...
第 N 周：/ky:architect → 基于新认知调整架构
```
